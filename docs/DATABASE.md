...
> Book Catalog Service — Database Design Documentation
...
|--------|------|----------|---------|-------------|-------------|
| `id` | BIGINT | NO | AUTO_INCREMENT | PRIMARY KEY | 主键 |
| `title` | VARCHAR(500) | NO | — | — | 书名 |
| `author` | VARCHAR(200) | NO | — | — | 作者 |
| `isbn` | VARCHAR(20) | YES | NULL | — | ISBN号 |
| `genre` | VARCHAR(50) | YES | NULL | — | 类型 |
| `price` | DECIMAL(10,2) | YES | NULL | — | 价格 |
| `description` | VARCHAR(2000) | YES | NULL | — | 描述 |
| `stock` | INT | NO | 0 | — | 库存 |
| `pages` | INT | NO | 0 | — | 页数 |
| `publisher` | VARCHAR(50) | YES | NULL | — | 出版社 |
| `publish_date` | DATETIME | YES | NULL | — | 出版日期 |
| `active` | TINYINT(1) | NO | 1 | — | 上下架状态: 1=上架, 0=下架 |
| `create_time` | DATETIME | NO | CURRENT_TIMESTAMP | — | 创建时间 |
| `update_time` | DATETIME | NO | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | — | 更新时间 |
| `version` | BIGINT | YES | 0 | Optimistic Lock | 乐观锁版本号 |
...
|------------|--------|------|---------|
| `PRIMARY` | `id` | PK | 主键索引 |
| `idx_book_title` | `title` | Normal | 按书名搜索 |
| `idx_book_author` | `author` | Normal | 按作者搜索 |
| `idx_book_genre` | `genre` | Normal | 按类型筛选 |
| `idx_book_isbn` | `isbn` | Normal | 按ISBN查询 |

> **Note**: ISBN 使用普通索引而非唯一索引，因为实体允许 ISBN 为 null，且应用层已在 Service 中做唯一性校验。如需数据库层面强约束，取消注释 DDL 中的 `UNIQUE INDEX` 行。

---

## DDL Script

> 完整 DDL 脚本同时保存在: [src/main/resources/db/schema.sql](../src/main/resources/db/schema.sql)
...
CREATE TABLE `t_book` (
    ...
    `genre`         VARCHAR(50)  DEFAULT NULL                     COMMENT 'Book genre',
    ...
    `active`        TINYINT(1)   NOT NULL DEFAULT 1               COMMENT 'Active: 1=active, 0=inactive',
...
-- ============================================================
-- 4. (Optional) Unique index on ISBN
-- ============================================================
...
# Verify
mysql -u root -p -e "USE bookcatalog; SELECT COUNT(*) AS total FROM t_book;"
```

## JPA Entity Mapping

The database table maps to the JPA entity at [Book.java](../src/main/java/com/eyebuy/bookcatalog/entity/Book.java):

| DB Column | Entity Field | JPA Annotation |
|-----------|-------------|----------------|
| `id` | `id` | `@Id @GeneratedValue(IDENTITY)` |
| `title` | `title` | `@Column(nullable=false, length=500)` |
| `author` | `author` | `@Column(nullable=false, length=200)` |
| `isbn` | `isbn` | `@Column(length=20)` |
| `genre` | `genre` | `@Column(length=50)` |
| `price` | `price` | `@Column(precision=10, scale=2)` |
| `description` | `description` | `@Column(length=2000)` |
| `stock` | `stock` | `@Column(nullable=false) @Builder.Default` |
| `pages` | `pages` | `@Column(nullable=false) @Builder.Default` |
| `publisher` | `publisher` | `@Column(length=50)` |
| `publish_date` | `publishDate` | — |
| `active` | `active` | `@Column(nullable=false) @Builder.Default` |
| `create_time` | `createTime` | `@Column(nullable=false, updatable=false)` |
| `update_time` | `updateTime` | `@Column(nullable=false) @PreUpdate` |
